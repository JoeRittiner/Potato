...
function startListening(client) {
    console.log("[Listen] Listening for Messages");
    if (!client.messageListener) {
        client.messageListener = (message) => {
            handleMessage(client, message);
        };
    } else {
        console.log("[Listen] Message listener already exists");
    }
...
function stopListening(client){
    console.log("[Listen] Stopping listening for Messages");
    if (client.messageListener) {
        client.off(Events.MessageCreate, client.messageListener);
        client.messageListener = null;
        console.log("[Listen] Message listener removed");
    } else {
        console.log("[Listen] No message listener found");
    }
}
...
async function handleMessage(client, message) {
    ...
    if (message.channel.type === ChannelType.DM) {
        ...
        console.log(`[Listen] Received a message in a voice channel: ${message.content}`);
        ...
        console.error(`[Listen] Received a message in an unknown channel type: ${message.channel.type}`);
...
function handleDMMessage(client, message) {
    console.log(`[Listen] Received DM from ${message.author.tag}: ${message.content}`);
    if (message.author.bot) return; // Ignore bot messages
    earPublish(client, message); // Publish the message to RabbitMQ for processing
}

function handleGuildTextMessage(client, message) {
    console.log(`[Listen] Received message: ${message.content} from ${message.author.tag} in guild ${message.guild ? message.guild.name : 'DM'} (channel ID: ${message.channel.id})`);
    if (message.author.bot) return; // Ignore bot messages
    earPublish(client, message); // Publish the message to RabbitMQ for processing
}